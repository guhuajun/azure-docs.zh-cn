...
description: 了解如何使用 Azure Active Directory 服务主体从 Azure 容器实例允许访问专用容器注册中心中的映像。
...
在本文中，了解如何创建和配置具有注册中心*拉取*权限的 Azure AD 服务主体。 然后，启动 Azure 容器实例 (ACI) 中的某个容器，以从专用注册中心拉取其映像，并使用服务主体进行身份验证。
